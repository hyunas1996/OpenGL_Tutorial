...
int main_my( void )
{
    ...
    // Open a window and create its OpenGL context
    window = glfwCreateWindow( 1024, 768, "Tutorial 04 - Color Cube", NULL, NULL);
    if( window == NULL ){
        fprintf( stderr, "Failed to open GLFW window. If you have an Intel GPU, they are not 3.3 compatible. Try the 2.1 version of the tutorials.\n" );
        getchar();
        glfwTerminate();
    ...
    glDepthFunc(GL_LESS);       // Accept fragment if is closer to the camera than the former one
    ...
    do{
        ...
        proj = glm::perspective(glm::radians(45.0f), (float)(1024/768), 0.1f, 100.0f);
